...
I learnt about [`nvm` (Node Version Manager)](https://nvm.sh/?ref=sc4r.net). It's a tool to configure different NodeJS environment versions to different projects. You can choose which NodeJS version to run the project on.
...
The next day, I was suggested the [asdfmovie playlist](https://www.youtube.com/watch?v=IYnsfV5N2n8&list=PL6HF94r1ogByYa2xFAXIE_1Pw-K0AU_Vd&index=1&ref=sc4r.net) on YouTube. I rewatched the whole playlist and enjoyed feeling nostalgic.
...
[`asdf`](https://asdf-vm.com/?ref=sc4r.net) is the same as `nvm`, but for many toolchains. It has support for NodeJS, PHP, Python, Go, Rust etc., it's even got support for `kubectl`, `minikube` for all the DevOps guys out there.
...
- Install `curl` and `git`. `sudo apt install curl git` on Debian. `brew install coreutils curl git` on macOS. Other OSes refer the [official docs](https://asdf-vm.com/guide/getting-started.html?ref=sc4r.net#_1-install-dependencies).
...
Please follow [offical documentation](https://asdf-vm.com/guide/getting-started.html?ref=sc4r.net#_3-install-asdf)
...
You can also look for plugins over [here](https://github.com/asdf-vm/asdf-plugins/tree/master/plugins?ref=sc4r.net)
